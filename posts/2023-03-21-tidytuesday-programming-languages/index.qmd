...
programming_lang %>%
  ...
  ggtitle("What are the popular symbols for commenting?") +
  theme_classic()
...
So now we have a bunch of summarized data in a single dataframe.
...
```{r}
join_madness %>%
  ggplot(aes(line_comment_token, n, size = log(number_of_users), color = log(number_of_users), label = title)) +
 # geom_point() +
  scale_y_log10() +
  geom_text() +
  theme_classic()
